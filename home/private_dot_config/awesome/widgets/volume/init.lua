...
local function TOG_MIC_CMD() return 'pactl set-source-mute @DEFAULT_SOURCE@ toggle' end
-- is there a waay to know current file without hardcoded?
local curfile = os.getenv("HOME") .. "/.config/awesome/widgets/volume/init.lua"
...
local function worker(user_args)
    ...
    function volume:togglemic()
        spawn.easy_async(TOG_MIC_CMD(), function() return end)
    end

    function volume:mixer()
        if mixer_cmd then
			if mixer_cmd == "pulsemixer" then
				local term = os.getenv("TERMINAL") or "kitty"
				spawn.easy_async(term .. ' ' ..mixer_cmd)
			else
    ...
    volume.widget:buttons(
		awful.util.table.join(
			...
			awful.button({ "Shift" }, 4, function() volume:inc(5) end),
			awful.button({ "Shift" }, 5, function() volume:dec(5) end),
			awful.button({}, 2, function() volume:mixer() end),
			awful.button({}, 1, function() volume:toggle() end),
			awful.button({ modkey }, 1, function() awful.spawn({"kitty", "-e", "nvim", curfile}) end)
		)
	)
