...
local opts = {
	command = "youtube-dl", -- if don't exist use 'yt-dlp' automagically
	--key bindings
...
require("mp.options").read_options(opts, "ytquality")
...
function show_menu()
	...
	msg.verbose(fmt("current %s-format: %s", opts.command, current_ytdl_format))
	local num_options = 0
	local options = {}
...
function download_formats()
	...
	-- Check if yt-dlp/youtube-dl exist or on $PATH
	local cs = exec({ opts.command, "--help" })
	if (cs < 0) then
		if (opts.command == "youtube-dl") or (opts.command == nil) or (opts.command == "") then
			opts.command = "yt-dlp"
			ytdl.path = "yt-dlp"
		else
	...
	mp.osd_message(fmt("fetching available formats with %s...", opts.command), 60)

	if not (ytdl.searched) then
		local ytdl_mcd = mp.find_config_file(opts.command)
		if not (ytdl_mcd == nil) then
			msg.verbose(fmt("found %s at: %s", opts.command, ytdl_mcd))
			ytdl.path = ytdl_mcd
		end
		ytdl.searched = true
	end
	...
	if (es < 0) or (json == nil) or (json == "") then
		mp.osd_message(fmt("%s: fetching formats failed...", opts.command), 1)
	...
	res = {}
	msg.verbose(fmt("%s succeeded!", opts.command))
